...
EPANET_ON.C -- This is a modified version of EPANET.C. Certain functions were
               added, but the normal functions of EPANET.C remain unchanged.
...
#define  EXTERN
#include "vars.h"
#include "toolkit_on.h"  // jevz changed on 3/06/03
...
/* Retrieve called-for parameter */
   switch (code)
   {
      ...
      case EN_TANKCURVE:   // jevz added on 25/3/03
         if (index <= Njuncs) return(251);
         *value = Tank[index-Njuncs].Vcurve;
         break;
...
/*** Updated 11/19/01 ***/
      ...
      case EN_PUMPHCURVE: // jevz added on 25/3/02
         if (Link[index].Type != PUMP)
            return (251);
         *value = Pump[PUMPINDEX(index)].Hcurve;
         break;

      case EN_PUMPECURVE: // jevz added on 25/3/02
         if (Link[index].Type != PUMP)
            return (251);
         *value = Pump[PUMPINDEX(index)].Ecurve;
         break;
...
/************************** END OF STANDARD EPANET.C *************************/

/* jevz
*************************************************************************

EPANET Functions Added To Extend Functionality of Toolkit
...
int DLLEXPORT  ENgetcurveindex(char *id, int *index)  // jevz added on 25/3/03
/*----------------------------------------------------------------
**  Input:   id     = curve ID
...
int DLLEXPORT ENgetcurveid(int index, char *id)  // jevz added on 25/3/03
/*----------------------------------------------------------------
...
*/
{
   strcpy(id,"");
   if (!Openflag) return(102);
   if (index < 1 || index > Ncurves) return(205);
   strcpy(id,Curve[index].ID);
   return(0);
}


int DLLEXPORT ENgetcurvelen(int index, int *len)  // jevz added on 25/3/03
/*----------------------------------------------------------------
**  Input:   index = index of curve
**  Output:  *len  = curve length (number of X-Y data points)
**  Returns: error code
**  Purpose: retrieves number of data points in a curve
**----------------------------------------------------------------
*/
{
   if (!Openflag) return(102);
...
int DLLEXPORT ENgetcurvetype(int index, int *type)  // jevz added on 25/3/03
/*----------------------------------------------------------------
**  Input:   index = index of curve
**  Output:  *type = curve type code (see TOOLKIT.H)
**  Returns: error code
**  Purpose: retrieves the curve type
**----------------------------------------------------------------
*/
{
   if (!Openflag) return(102);
   if (index < 1 || index > Ncurves) return(205);
   *type = Curve[index].Type;
   return(0);
}


int DLLEXPORT ENgetcurvepoint(int index, int pntindex, float *xvalue, float *yvalue)  // jevz added on 25/3/03
/*----------------------------------------------------------------
**  Input:   index   = index of curve
**           ptindex = index of point
**  Output:  *xvalue = point's x value
**           *yvalue = point's y value
**  Returns: error code                              
**  Purpose: retrieves a data point at a specific index
**----------------------------------------------------------------
*/
{
   if (!Openflag) return(102);
   if (index < 1 || index > Ncurves) return(205);
   if (pntindex < 1 || pntindex > Curve[index].Npts) return(251);
   *xvalue = Curve[index].X[pntindex-1];
   *yvalue = Curve[index].Y[pntindex-1];
   return(0);
}


int  DLLEXPORT  ENsetcurve(int index, float *x, float *y, int n)  // jevz added on 25/3/03
/*----------------------------------------------------------------
**   Input:   index = curve index
**            *x    = array of curve x values
**            *y    = array of curve y values
**            n     = number of points in curve
**   Output:  none
**   Returns: error code                              
**   Purpose: sets points for a specific curve
**----------------------------------------------------------------
*/
{
   int j;
...
/* Check for valid arguments */
   if (!Openflag) return(102);
   if (index <= 0 || index > Ncurves) return(205);
   if (n <= 0) return(202);

/* Re-set number of curves & reallocate memory for points */
   Curve[index].Npts = n;
   Curve[index].X = (float *) realloc(Curve[index].X, n*sizeof(float));
   Curve[index].Y = (float *) realloc(Curve[index].Y, n*sizeof(float));
   if (Curve[index].X == NULL || Curve[index].Y == NULL) return(101);
...
// jevz added on 25/3/02
/*----------------------------------------------------------------
**  Input:   index   = curve index
**           ptindex = point index in curve
**           x       = point's x value
**           x       = point's y value
**  Output:  none
**  Returns: error code
**  Purpose: sets values for a specific point on a specific curve
**----------------------------------------------------------------
*/
{
   if (!Openflag) return(102);
   if (index  <= 0 || index  > Ncurves) return(205);
   if (ptindex <= 0 || ptindex > Curve[index].Npts) return(251);
   Curve[index].X[ptindex-1] = x;
   Curve[index].Y[ptindex-1] = y;
   return(0);
}
